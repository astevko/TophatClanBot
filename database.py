...
async def insert_default_ranks(db):
    ...
    for rank_order, rank_name, points_required, roblox_rank_id in default_ranks:
        await db.execute("""
            INSERT OR IGNORE INTO rank_requirements 
            (rank_order, rank_name, points_required, roblox_group_rank_id)
            VALUES (?, ?, ?, ?)
        """, (rank_order, rank_name, points_required, roblox_rank_id))
...
async def get_next_rank(current_rank_order: int) -> Optional[Dict[str, Any]]:
